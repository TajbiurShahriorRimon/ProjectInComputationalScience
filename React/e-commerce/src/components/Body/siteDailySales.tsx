...
class SiteDailySales extends Component<any, any>{
    state = {
        result: [],
        loading: true,
    }
    ...
    async componentDidMount() {
        var year = window.location.href.split('/').slice(-2)[0];
        var month = window.location.href.split('/').slice(-1)[0];
        //alert(window.location.pathname.split("/").pop());
        const resp = await axios.get(`${base_url}order/dailySales/`+year+'/'+month);
        console.log(resp);
